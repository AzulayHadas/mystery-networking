...
participants_data = {}
qr_mapping = {}
qr_data_df = None  # Cached QR mapping DataFrame
...
def load_participant_data():
    ...
    global participants_data, qr_mapping, qr_data_df, email_to_userid, target_assignments, target_assignments_df
    
    try:
        # Load enhanced QR mapping with genre and job title
        qr_file = os.path.join(BASE_DIR, 'qr_codes', 'qr_mapping.csv')
        if os.path.exists(qr_file):
            qr_data_df = pd.read_csv(qr_file)
            qr_mapping = dict(zip(qr_data_df['user_id'], qr_data_df['email']))
            email_to_userid = dict(zip(qr_data_df['email'], qr_data_df['user_id']))
        ...
        if db is None:
            ...
            if target_id:
                ...
                if target_email and target_email in participants_data:
                    ...
                    # Get job title from cached QR mapping
                    job_title = "Unknown Role"
                    try:
                        if qr_data_df is not None:
                            target_row = qr_data_df[qr_data_df['user_id'] == target_id]
    ...
    try:
        ...
        if db is None:
            ...
            if scanned_email and scanned_email in participants_data:
                ...
                try:
                    if qr_data_df is not None:
                        scanned_row = qr_data_df[qr_data_df['user_id'] == scanned_id]
...
def get_all_missions():
    ...
    try:
        ...
        if db is None:
            ...
            # Build QR data lookup from cached DataFrame
            qr_data = {}
            if qr_data_df is not None:
                for _, row in qr_data_df.iterrows():
                    qr_data[row['user_id']] = {
...
def upload_config():
    ...
    global participants_data, qr_mapping, email_to_userid, target_assignments
...
if __name__ == '__main__':
    ...
    print(f"📊 Admin Panel: Open frontend/admin.html in browser")
    print(f"🎯 Participant App: Open frontend/participant.html in browser")
    print(f"📺 Dashboard: Open frontend/dashboard.html in browser\n")
